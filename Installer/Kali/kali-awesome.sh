	...
	if [ ! -f $tarball ]; then
		...
		*)
		...
		if [ $archurl == "arm64" ]; then
			wget "https://github.com/Muntader-N/My-ubuntu/releases/download/kali-arm64-tarball/kali-rootfs-arm64.tar.xz" -O $tarball
		else
			wget "https://github.com/Muntader-N/My-ubuntu/blob/master/Rootfs/Kali/${archurl}/kali-rootfs-${archurl}.tar.xz?raw=true" -O $tarball
		fi
	fi
	cur=`pwd`
	mkdir -p "$folder"
